...
namespace Selfiesh
{
    ...
    {       
        ...
        public CPcapNet pcaparp;
        public CPcapNet pcapredirect;
        ...
        public Arp(NetworkInterface nic, Socket Socket)
        {
            this.Socket = Socket;
            NIC = nic;
            int num = 0;
            if (0 < nic.GetIPProperties().UnicastAddresses.Count)
            {
                do
                {
                    if (!Convert.ToString(NIC.GetIPProperties().UnicastAddresses[num].Address.AddressFamily).EndsWith("V6"))
                    {
                        localIP = NIC.GetIPProperties().UnicastAddresses[num].Address.GetAddressBytes();
                        netmask = NIC.GetIPProperties().UnicastAddresses[num].IPv4Mask.GetAddressBytes();
                    }
                    num++;
                }
                while (num < NIC.GetIPProperties().UnicastAddresses.Count);
            }
            localMAC = NIC.GetPhysicalAddress().GetAddressBytes();
            if (NIC.GetIPProperties().GatewayAddresses.Count > 0)
            {
                routerIP = NIC.GetIPProperties().GatewayAddresses[0].Address.GetAddressBytes();
            }
           ...
           while(true)
            {
                ...
                if (Socket.areValuesEqual(array, localMAC))
                {
                    continue;
                }
                byte b = pkt_data[21];
                if (b.ToString().CompareTo("2") == 0)
                {
                  ...
                  //  System.Net.NetworkInformation.Ping ping = new System.Net.NetworkInformation.Ping();
                    ...
                    pC.capUp = 0;
                    pC.isLocalPc = false;
                    pC.name = GetHostName(pC.ip.ToString());
                    ...
                    if (Socket.areValuesEqual(array2, routerIP))
                    {
                        routerMAC = array;
                        pC.isGateway = true;
                    }
        ...
        {
            string text = null;
            if (pcaparp.nicHandle == IntPtr.Zero && !pcaparp.pcapnet_openLive(NIC.Id, 65535, 0, 1, text))
            {               
                return -1;
            }
        ...
        public void findMac(string ip)
        {
            ...
            byte[] addressBytes = Socket.getIpAddress(ip).GetAddressBytes();
        ...
        private void redirector()
        {
            ...
            Client router = Socket.GetRouter();
            if (router != null)
            {
            ...
            if (isRedirecting)
            {
                do
                {
                    ...
                    if (Socket.areValuesEqual(array, localMAC))
                    {
                        ...
                        if (Socket.areValuesEqual(array2, localIP))
                        {
                            Socket.GetLocalPC().nbPacketSentSinceLastReset += (int)pkt_hdr.caplen;
                        }
                        continue;
                    }
                    if (Socket.areValuesEqual(array, routerMAC))
                    {
                        Array.Copy(pkt_data, 30, array3, 0, 4);
                        if (Socket.areValuesEqual(array3, localIP))
                        {
                            Socket.GetLocalPC().nbPacketReceivedSinceLastReset += (int)pkt_hdr.caplen;
                            continue;
                        }
                        Client pCFromIP = Socket.GetClientFromIP(array3);
                        if (pCFromIP != null)
                        {
                    ...
                    Client pCFromMac = Socket.GetClientFromMAC(array);
                    if (pCFromMac != null)
                    {
        ...
        {
            Client pCFromIP = Socket.GetClientFromIP(ip1.GetAddressBytes());
            Client pCFromIP2 = Socket.GetClientFromIP(ip2.GetAddressBytes());
        ...
        public void completeUnspoof()
        {
            ...
            int num = 0;
            if (0 < Socket.Pool.Count)
            {
                do
                {
                    UnSpoof(((Client)Socket.Pool[num]).ip, router.ip);
                    num++;
                }
                while (num < Socket.Pool.Count);
            }
        }
    }
}
